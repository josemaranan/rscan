...
$sqlQuery  = " DELETE 
					FROM Rnet.dbo.prmWorkflowPositions_V1 
			   WHERE 
...
foreach($_REQUEST[ckhBVox] as $keyEx=>$ValueEx)
{
		...
		if(!empty($ValueEx))
		{
			...
			$sqlQuery .= " INSERT INTO Rnet.dbo.prmWorkflowPositions_V1 
							VALUES (
										'".trim($vArray[1])."',
										'".trim($vArray[0])."'
									)";
...
{
	header("Location:assignPositionstoWidgets_configure_IT.php?ddlWorkFlows=".$ddlWorkFlows."&ddlDepartment=".urlencode($ddlDepartment));
}
else
{
	header("Location:assignPositionstoWidgets_search_IT.php");	
}
exit();
?>