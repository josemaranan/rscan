...
//		$commonListBox->resetProperties();				
	...
	case 'viewPositionsList':
		...
		if(!empty($positionIDString) && !empty($workFlowIDString))
		{
			$sqlQuery = " SELECT 
								...
								country
						FROM 
								Rnet.dbo.prmWorkFlowPositions_backup_rk_20150525 (nolock) 
						WHERE 
		...
		//Country listing
		echo $htmlTagObj->openTag('div', 'style="text-align:left;"');
		echo '<b>Select Country:</b> &nbsp;&nbsp;&nbsp;';
		$sqlQuery = " SELECT DISTINCT country FROM results..ctlLocations (NOLOCK) WHERE country NOT IN ('Dominican Republic','Panama') AND country IS NOT NULL";
		$resultsSet = $RDSObj->execute($sqlQuery);
		$numRows = $RDSObj->getNumRows($resultsSet);
		if ($numRows >= 1)
		{
			while($countriesArr = mssql_fetch_assoc($resultsSet))
			{
				echo ' ' . $countriesArr['country'] . ' ';
				//$countryArray[$countriesArr['country']] = 	$countriesArr['country'];
				$htmlTextElement->type	= 'checkbox';
				$htmlTextElement->name	= 'workFlowCountry[]';
				$htmlTextElement->id	= 'workFlowCountry'.$countriesArr['country'];
				$htmlTextElement->value	= $countriesArr['country'];
				if(in_array($countriesArr['country'], $existingCountryArray))
				{
					$htmlTextElement->checked	= 'checked';
				}
				echo $countryCheckBoxString =  $htmlTextElement->renderHtml();
				echo '&nbsp;&nbsp;&nbsp;';
				$htmlTextElement->resetProperties();
			}
		}
	...
	case 'configurePositions':
		...
		$sqlQuery  = " DELETE 
							FROM Rnet.dbo.prmWorkFlowPositions_backup_rk_20150525 
					   WHERE 
		...
		foreach($_REQUEST[ckhBVox] as $keyEx=>$ValueEx)
		{
			...
			if(!empty($ValueEx))
			{
				...
				if(!empty($countryVal))
				{
					...
					{
						$sqlQuery .= " INSERT INTO Rnet.dbo.prmWorkFlowPositions_backup_rk_20150525
										VALUES (
													'".trim($vArray[1])."',
													'".trim($vArray[0])."',
													'".trim($val)."'
