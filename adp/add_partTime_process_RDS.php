 ...
 WITH (NOLOCK) WHERE employeeID = '$employeeID' AND effectiveDate = '$startDate'  ";
	...
	{	
		...
		if($rst)
		{
			...
			header("Location: index_test.php?hdnEmployeeID=".$employeeID."&adpMode=hr&adpTask=empFullTime&error=AddPartTimeSuccess&activeLink=20");	
			exit;
		}
	}
	else
	{
		...
		header("Location: index_test.php?hdnEmployeeID=".$employeeID."&adpMode=hr&adpTask=empFullTime&error=AddPartTimeError");	
		exit;
	}
} 
