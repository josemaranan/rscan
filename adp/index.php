...
$cssJsArray = array('CSS'=>array('readiNetAll.css', 'adpcss.css' , 'modalwindowzindex.css','dhtmlgoodies_calendar.css?random=20051112', '../../RNetIncludes/css/tipped/tipped.css'), 'JS'=>array($dhtmlCalendar , 'table.js','jquery-1.9.1.min.js','adp/includes/adpdymicwidthHeightv.js', 'adp/includes/ajax.js', 'adp/includes/adpJavascript.js', '../../RNetIncludes/js/tipped/tipped.js', '../../RNetIncludes/js/tipped/helpTooltip.js'));
...
echo '<script type="text/javascript" src="/Include/javascript/commonJsFunctions.js"></script>';
/* Step 3 */
...
function linkToAdpc(msg,isReason,empID)
{
	var html = msg;	
	var getTempId = new Date().getTime();
	
	$('body').append('<div id="'+getTempId+'" style="display:none;">'+html+'</div>');
	$("#"+getTempId).dialog({
		 height:'auto',
		 width:600,										   
		 modal:true,
	...
	if(isReason == 'Required')
	{
		generateADPCFile(empID,'CHANGES',getTempId);
	}
}
</script>
<?php 
if(isset($_REQUEST['error']))
{
	...
	if($errorMessage == 'AddSalarySuccess' || $errorMessage == 'EditSalarySucess')
	{
		$empID = $_REQUEST['hdnEmployeeID'];
		$changeReason = $_REQUEST['changeReason'];
		
		$message = $employeeeMaintenanceObj->Errordefines[$errorMessage];		
		if($changeReason == 'Required')
		{	
			$message .= '<br/>You have updated the pay rate in a previously finalized pay period. These changes need to be pushed to ADP';
			$message .= '<br/><img src="/Include/images/progress.gif" style="vertical-align:middle;"> &nbsp; ADPC file is generating.';	
		}
	?>
		<script type="text/javascript">
		linkToAdpc('<?php echo $message; ?>', '<?php echo $changeReason; ?>','<?php echo $empID; ?>');
		</script>
	<?php
	}
	else {
	...
	<script type="text/javascript">
	var changeReason = '<?php echo $_REQUEST['changeReason']; ?>';
	var employeeID = '<?php echo $_REQUEST['hdnEmployeeID']; ?>'; 
	var adpTask = '<?php echo $_REQUEST['adpTask']; ?>';
	
	if(changeReason == 'Y')
	{
		if(adpTask == 'empSupervisor')
		{
			showADPCdialog_ADP(employeeID,'supervisor');
		}
		else if(adpTask == 'empDates')
		{
			showADPCdialog_ADP(employeeID,'date(s)');
		}
...
$(document).ready(function(){						   		
	if( $( "DIV.scrollingdatagrid " ).length > 0)
	{ 
