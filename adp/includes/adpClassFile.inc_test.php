...
class ADPEmployeeClass extends ADPPayroll {
	...
	public $accessPositions = '4,22,105,17,42,163';
	...
	function setUSLocations()
	{
		...
		$sqlQuery = "SELECT 
				...
				ORDER BY 
					description ";
		$resultsSet = $this->execute($sqlQuery);
		...
		if($getCountryName=='United States of America')
		{
			...
			$sqlMissingData = " 
						...
						AND
							d.isPrimary = 'Y'
						--LEFT JOIN
							--RNet.dbo.prmEmployeeCareerHistory f WITH (NOLOCK)
						--ON
			...
			$rstMissingData = $this->execute($sqlMissingData);
