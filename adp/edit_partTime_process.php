...
if($_REQUEST)
{
	...
	if($num == 0)
	{
		if(!empty($employeeID))
		{
			...
			if($rstQry)
			{
				...
				header("Location: index.php?hdnEmployeeID=".$employeeID."&adpMode=hr&adpTask=empFullTime&error=EditPartTimeSuccess&activeLink=20");
	...
	else
	{
		...
		header("Location: index.php?hdnEmployeeID=".$employeeID."&fullTimePartTimeEffectiveDate=".$pk_startDate."&adpMode=hr&adpTask=empFullTimeEdit&error=EditPartTimeError");
