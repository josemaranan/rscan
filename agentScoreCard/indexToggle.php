...
$cssJsArray = array('CSS'=>array('agentScoreNew.css','dhtmlgoodies_calendar.css?random=20051112', 'modalwindowzindex.css'), 'JS'=>array('dhtmlgoodies_calendar.js?random=20060118' , 'table.js','agentScoreCard/jquery.min.js', 'agentScoreCard/rnetCharts.js' ,'agentScoreCard/validations.js','agentScoreCard/dymicwidthHeightv.js'));
...
$topLevelHeading = 'Agent scorecard';
...
/*$trStryleArray = array('0'=>'<tr style="height:20px; background-color:#D0D8E8;" class="main">', '1'=>'<tr style="height:20px; background-color:#E9EDF4;" class="hidden">', '3'=>'<tr style="height:20px; background-color:#999999;" class="hidden" >', '4'=>'<tr style="height:20px; background-color:#E9EDF4;">');*/
...
<form name="frmTopLvel" id="frmTopLvel" method="post" action="<?php echo $_SERVER['PHP_SELF'];?>" style="padding:0px; margin:0px;">
    ...
    <div id="wellCareImg">
    	<? if(strtolower($_SESSION[agentScoreClient]) == 'wellcare')
		{
			?>
			<img src="includes/images/wellcare.png" alt="Wellcare" width="200" height="50">
	<?
		}
		else if(strtolower($_SESSION[agentScoreClient]) == 'helio')
		{
			?>
			<img src="includes/images/Sprint.jpg" alt="Sprint" width="100" height="50">
			<?
		}
		else
		{?>
			<img src="includes/images/comcastScoreCard.png" alt="Comcast" width="100" height="50">
		<? }
		?>
    </div>
</form>
...
<div id = "midContent" class="outer">
	...
	<div id = "legend" style="height:35px;">
    	<div id="legendHeading" style="text-align:left; padding-left:25px; background-image:url(includes/images/plus.gif); background-repeat:no-repeat; background-position:left;" class="locked" onclick="return toggleDivs('legendContent', this.id); return false;" title="expand"> Legend</div>
	...
	<div id = "General">		
        	...
        	<?php 
				...
				echo '<div id = "generalHeading" style="text-align:left; padding-left:20px; background-image:url(includes/images/plus.gif); background-repeat:no-repeat; background-position:left;" class="locked" onclick="return toggleDivs(\'generalBodyContent\', this.id); return false;" title="expand">'.$sectionTitle.'</div>';
				echo '<div id = "generalBodyContent" style="display:none">';
	...
	<div class="scrollingdatagridNew" id="scrollingdatagrid" visible="true">
		...
		for($idx=1; $idx< $cntBelowTbls; $idx++)
		{
			...
			if($countOf>0)
			{
				...
				echo '<div class="sectionHeading" id="div'.$idx.'" style="text-align:left; padding-left:20px; background-image:url(includes/images/plus.gif); background-repeat:no-repeat; background-position:left;" class="locked" onclick="return toggleDivs(\'exPandDiv'.$idx.'\', this.id); return false;" title="expand">'.$sectionTitle.'</div>';
				echo '<div class="sectionContent" id="exPandDiv'.$idx.'" style="display:none">';
				unset($displayEachSectionData);
				$displayEachSectionData =  $agentScoreObj->getConentBySectionWise($getSections, 
																		  $dataSections, 
																		  $allSectionsData, 
