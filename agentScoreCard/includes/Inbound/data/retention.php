...
foreach($requiredData as $rrow)
{
	if($rrow['LOB']==$lob && $rrow['Date']!='Subtotals')
	{
		$date[] = $rrow['Date'];
		$saves[] = $rrow['Saves'];
		$missedSaves[] = $rrow['Missed Saves'];
		$Retention[] = number_format($rrow['Retention'],2,'.','');
		
		$retentionGoal[] = $agentScoreObj->numberFormatReturn($rrow['Retention Goal']);
...
$Jsaves = $agentScoreObj->JsonConvertion($saves, 'number');
$JmissedSaves = $agentScoreObj->JsonConvertion($missedSaves, 'number');
$JRetention = $agentScoreObj->JsonConvertion($Retention, 'number');
$JretentionGoal = $agentScoreObj->JsonConvertion($retentionGoal, 'number');
...
?>
		...
		yAxis:[
				...
				labels:{
					formatter:function(){return Highcharts.numberFormat(this.value,0)}},min:0}],legend:{verticalAlign:'top',marginTop:25},tooltip:{formatter:function(){return'<b>'+this.x+'</b><br/>'+this.series.name+': '+this.y+(this.series.name=='Retention'?'%':'')}},plotOptions:{column:{stacking:'normal'}},series:[{name:'Missed Saves',type:'column',yAxis:0,data:<?php echo $JmissedSaves;?>},{name:'Saves',type:'column',yAxis:0,data:<?php echo $Jsaves;?>},{name:'Retention %',type:'line',yAxis:1,data:<?php echo $JRetention;?>},{name:'Retention Goal %',type:'line',yAxis:1,data:<?php echo $JretentionGoal;?>}]})});
