...
foreach($requiredData as $rrow)
{
	if($rrow['LOB']==$lob && $rrow['Date']!='Subtotals')
	{
		...
		$FCRGoal[] = $agentScoreObj->numberFormatReturn($rrow['FCR Goal']);
		
		$firstCallResolution[] = $rrow['First Call Resolution'];
		$repeatCallers[] = $rrow['Repeat Callers'];
		$FCR[] = $agentScoreObj->numberFormatReturn($rrow['FCR']);
		$count++;
	}
}
...
$JFCRGoal = $agentScoreObj->JsonConvertion($FCRGoal, 'number');//$agentScoreObj->JsonConvertion($FCRGoal, 'number');
$JfirstCallResolution = $agentScoreObj->JsonConvertion($firstCallResolution, 'number');
$JrepeatCallers = $agentScoreObj->JsonConvertion($repeatCallers, 'number');
$JFCR = $agentScoreObj->JsonConvertion($FCR, 'number');

//$JFCRGoal2 = str_replace('%', '', $JFCRGoal);
...
?>
		...
		var chart;
                ...
                labels: {
					...
					series: [
					{
						name: 'Repeat Callers', // Dynamic
						//color: '#4E81BD', // Dynamic
						type: 'column',
						yAxis: 0,
						data: <?php echo $JrepeatCallers;?>
						
					},			 
					{
						name: 'First Call Resolution', // Dynamic
						//color: '#4E81BD', // Dynamic
						type: 'column',
						yAxis: 0,
					...
					{
						name: 'FCR Goal %', // Dynamic
						//color: '#4E81BD', // Dynamic
						type: 'line',
						yAxis: 1,
						data: <?php echo $JFCRGoal;?>
