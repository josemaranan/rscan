...
foreach($requiredData as $rrow)
{
	...
	{
		$dates[] = $rrow['Date'];
		$Opportunities[] = $rrow['Opportunities'];
		$appointmentsScheduled[] = $rrow['Appointments Scheduled'];
		$appointmentsSet[] = $agentScoreObj->numberFormatReturn($rrow['Appointments Set']);
		
		$careGapsGoal[] = $agentScoreObj->numberFormatReturn($rrow['Care Gaps Goal']);
...
$JOpportunities = $agentScoreObj->JsonConvertion($Opportunities, 'number');
$JappointmentsScheduled = $agentScoreObj->JsonConvertion($appointmentsScheduled, 'number');
$JappointmentsSet = $agentScoreObj->JsonConvertion($appointmentsSet, 'number');

$JcareGapsGoal = $agentScoreObj->JsonConvertion($careGapsGoal, 'number');
	...
	$("#LegendTable").append(str);
		...
		var chart;
				...
				min:0
					...
					{
						name: 'Appointments Schedule', // Dynamic
						//color: '#4E81BD', // Dynamic
						type: 'column',
						yAxis: 0,
						data: <?php echo $JappointmentsScheduled;?>
						
					},
					{
						name: 'Care Gap Opportunities', // Dynamic
						//color: '#4E81BD', // Dynamic
						type: 'column',
						yAxis: 0,
						data: <?php echo $JOpportunities;?>
						
					},
					{
						name: 'Appointments Set %', // Dynamic
						//color: '#4E81BD', // Dynamic
						type: 'line',
						yAxis: 1,
						data: <?php echo $JappointmentsSet;?>
						
					},
					{
						name: 'Care Gaps Goal %', // Dynamic
						//color: '#4E81BD', // Dynamic
						type: 'line',
						yAxis: 1,
						data: <?php echo $JcareGapsGoal;?>
