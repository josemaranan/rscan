...
<tr>
    ...
    <th class="ColumnHeader1">AHT Goal</th>
    ...
    <th class="ColumnHeader1">ACW Time</th>
		...
		if(!empty($requiredData))
		{
			...
			{ 
				...
				?>
							...
							if(!empty($requiredDataVal['AHT Goal']))
							{
								$agentScoreObj->numberFormat($requiredDataVal['AHT Goal']);
							}
							else
							{
								echo '&nbsp;';	
							...
							{?>
                                ...
                                { ?>
                                         <img src="includes/images/<?php echo $iconf;?>" border="0" style="width:40px; height:35px; cursor:hand;" title="Click here to view chart" onclick="return populateDetails('<?php echo $type;?>', '<?php echo $measure;?>' , '<?php echo $period;?>' , '<?php echo $employeID;?>', '<?php echo $startDate;?>' , '<?php echo $endDate;?>' , 'dataPie' , '' ); return false;" />
                                <?php }
							...
							<td style="text-align:center;"><?php 
								if(!empty($requiredDataVal['Calls Handled']))
								{
									echo $requiredDataVal['Calls Handled'];
								}
								else
								{
									echo '&nbsp;';	
							...
							<td style="text-align:center;"><?php 
							if(!empty($requiredDataVal['AHT']))
							{
								$agentScoreObj->numberFormat($requiredDataVal['AHT']);
							}
							else
							{
								echo '&nbsp;';	
							...
							<td style="text-align:center;"><?php 
								if(!empty($requiredDataVal['ACD Time']))
								{
									$agentScoreObj->numberFormat($requiredDataVal['ACD Time']);
								}
								else
								{
									echo '&nbsp;';		
							...
							<td style="text-align:center;"><?php 
							if(!empty($requiredDataVal['Hold Time']))
							{
								$agentScoreObj->numberFormat($requiredDataVal['Hold Time']);
							}
							else
							{
								echo '&nbsp;';		
							...
							?></td>
							<td style="text-align:center;"><?php 
							if(!empty($requiredDataVal['ACW Time']))
							{
								$agentScoreObj->numberFormat($requiredDataVal['ACW Time']);
							}
							else
							{
								echo '&nbsp;';		
