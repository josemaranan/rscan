...
foreach($requiredData as $rrow)
{
	if($rrow['LOB']==$lob && $rrow['Date']!='Subtotals')
	{
		...
		$TakeBacks[] = $agentScoreObj->numberFormatReturn($rrow['TakeBacks']);
		$CallsHandled[] = $agentScoreObj->numberFormatReturn($rrow['CallsHandled']);
		$Upgrade[] = $agentScoreObj->numberFormatReturn($rrow['Upgrade']);
...
$JTakeBacks = $agentScoreObj->JsonConvertion($TakeBacks, 'number');
$JCallsHandled = $agentScoreObj->JsonConvertion($CallsHandled, 'number');
$JUpgrade = $agentScoreObj->JsonConvertion($Upgrade, 'number');
	...
	$("#LegendTable").append(str);
		...
		var chart;
                ...
                labels: {
					...
					{
						name: 'Take Backs', // Dynamic
						//color: '#4E81BD', // Dynamic
						type: 'line',
						yAxis: 0,
						data: <?php echo $JTakeBacks;?>
					...
					{
						name: 'Upgrade', // Dynamic
						//color: '#4E81BD', // Dynamic
						type: 'line',
						yAxis: 0,
						data: <?php echo $JUpgrade;?>
