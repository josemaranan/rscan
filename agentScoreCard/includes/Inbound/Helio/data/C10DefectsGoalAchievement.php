...
foreach($requiredData as $rrow)
{
	...
	{
		$dates[] = $rrow['Date'];
		$CallsHandled[] = $agentScoreObj->numberFormatReturn($rrow['CallsHandled']);
		$DEF[] = $agentScoreObj->numberFormatReturn($rrow['DEF']);
		$OPPT[] = $agentScoreObj->numberFormatReturn($rrow['OPPT']);
		$C10DS[] = $agentScoreObj->numberFormatReturn($rrow['C10DS']);
		$C10DEF[] = $agentScoreObj->numberFormatReturn($rrow['C10DEF']);
		$C10OPP[] = $agentScoreObj->numberFormatReturn($rrow['C10OPP']);
		$C10_TARGET[] = $agentScoreObj->numberFormatReturn($rrow['C10_TARGET']);
...
$JDEF = $agentScoreObj->JsonConvertion($DEF, 'number');
$JOPPT = $agentScoreObj->JsonConvertion($OPPT, 'number');
$JC10DS = $agentScoreObj->JsonConvertion($C10DS, 'number');
$JC10DEF = $agentScoreObj->JsonConvertion($C10DEF, 'number');
$JC10OPP = $agentScoreObj->JsonConvertion($C10OPP, 'number');
$JC10_TARGET = $agentScoreObj->JsonConvertion($C10_TARGET, 'number');
		...
		var chart;
                ...
                labels: {
					...
					},
					{
						...
						type: 'line',
						yAxis: 0,
						data: <?php echo $JC10OPP;?>
						
					},
					{
						name: 'C10_TARGET', // Dynamic
						//color: '#4E81BD', // Dynamic
						type: 'line',
						yAxis: 0,
						data: <?php echo $JC10_TARGET;?>
