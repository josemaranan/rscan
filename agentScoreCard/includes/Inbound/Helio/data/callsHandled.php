...
<?php exit(); }
	...
	foreach($requiredData as $rrow)
	{
		...
		{
			$date[] = $rrow['Date'];
			$callsHandled[] = $rrow['CallsHandled'];
			$AHT[] = $agentScoreObj->numberFormatReturn($rrow['AHT']);
			$AHTGoal[] = $agentScoreObj->numberFormatReturn($rrow['AHT Goal']);
			$talkTime[] = $agentScoreObj->numberFormatReturn($rrow['ACDTime']);
			$holdTime[] = $agentScoreObj->numberFormatReturn($rrow['HoldTime']);
			$ACWTime[] = $agentScoreObj->numberFormatReturn($rrow['WrapTime']);
