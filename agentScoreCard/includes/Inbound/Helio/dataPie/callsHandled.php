...
if($measure=='AHT')
{
		...
		foreach($requiredData as $rrow)
		{
			if($rrow['LOB']=='Totals')
			{
				...
				$talkTime['Talk Time'] = $agentScoreObj->numberFormatReturn($rrow['ACDTime'],2);
				$holdTime['Hold Time'] = $agentScoreObj->numberFormatReturn($rrow['HoldTime'],2);
				$ACWTime['ACW Time'] = $agentScoreObj->numberFormatReturn($rrow['WrapTime'],2);
...
else 
{
	...
	foreach($requiredData as $rrow)
	{
		if($rrow['initialDisplay']=='1' && $rrow['LOB']!='Totals')
		{
			//$callsHandled['Calls Handled'] = $agentScoreObj->numberFormatReturn($rrow['Calls Handled'],2);
			//$AHT['AHT'] = $agentScoreObj->numberFormatReturn($rrow['AHT'],2);
			$lobLevel[$rrow['LOB']] = $rrow['CallsHandled'];
			/*
