...
<?php exit(); }
	...
	foreach($requiredData as $rrow)
	{
		if($rrow['LOB']=='Totals')
		{
			...
			$ADVN_TOP_BOX['ADVN_TOP_BOX'] = $agentScoreObj->numberFormatReturn($rrow['ADVN_TOP_BOX'],2);
			$ADVN_BTM_BOX['ADVN_BTM_BOX'] = $agentScoreObj->numberFormatReturn($rrow['ADVN_BTM_BOX'],2);
...
$JADVN_TOP_BOX = $agentScoreObj->JsonKeyValueConvertion($ADVN_TOP_BOX, 'number');
$JADVN_BTM_BOX = $agentScoreObj->JsonKeyValueConvertion($ADVN_BTM_BOX, 'number');
...
?>
		...
		var chart;$(document).ready(function(){chart=new Highcharts.Chart({chart:{renderTo:'container',marginRight:50,defaultSeriesType: 'pie'},credits:{enabled:false},title:{text:''},tooltip: {
                ...
                type: 'pie',
                name: 'Browser share',
                data: [<?php echo $JCallsHandled;?>,<?php echo $JADVN_TOP_BOX;?>,<?php echo $JADVN_BTM_BOX;?>]
            }]})});
