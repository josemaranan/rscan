...
<?php exit(); }
	...
	foreach($requiredData as $rrow)
	{
		if($rrow['LOB']=='Totals')
		{
			...
			$DEVPRO['DEVPRO'] = $agentScoreObj->numberFormatReturn($rrow['DEVPRO'],2);
...
$JDEVPRO = $agentScoreObj->JsonKeyValueConvertion($DEVPRO, 'number');
...
?>
		...
		var chart;$(document).ready(function(){chart=new Highcharts.Chart({chart:{renderTo:'container',marginRight:50,defaultSeriesType: 'pie'},credits:{enabled:false},title:{text:''},tooltip: {
                ...
                type: 'pie',
                name: 'Browser share',
                data: [<?php echo $JUPG;?>,<?php echo $JDEVPRO;?>,<?php echo $JGA;?>]
            }]})});
