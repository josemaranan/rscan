...
<?php exit(); }
	...
	foreach($requiredData as $rrow)
	{
		if($rrow['LOB']=='Totals')
		{
			...
			$TakeBacks['TakeBacks'] = $agentScoreObj->numberFormatReturn($rrow['TakeBacks'],2);
			$CallsHandled['CallsHandled'] = $agentScoreObj->numberFormatReturn($rrow['CallsHandled'],2);
			$Upgrade['Upgrade'] = $agentScoreObj->numberFormatReturn($rrow['Upgrade'],2);
...
$JTakeBacks = $agentScoreObj->JsonKeyValueConvertion($TakeBacks, 'number');
$JCallsHandled = $agentScoreObj->JsonKeyValueConvertion($CallsHandled, 'number');
$JUpgrade = $agentScoreObj->JsonKeyValueConvertion($Upgrade, 'number');
...
?>
		...
		var chart;$(document).ready(function(){chart=new Highcharts.Chart({chart:{renderTo:'container',marginRight:50,defaultSeriesType: 'pie'},credits:{enabled:false},title:{text:''},tooltip: {
                ...
                type: 'pie',
                name: 'Browser share',
                data: [<?php echo $JUPG;?>,<?php echo $JTakeBacks;?>,<?php echo $JCallsHandled;?>,<?php echo $JUpgrade;?> ]
            }]})});
