...
<?php exit(); }
	...
	foreach($requiredData as $rrow)
	{
		if($rrow['LOB']=='Totals')
		{
			$Opportunities['Opportunities'] = $rrow['Opportunities'];
			$AppointmentsScheduled['Appointments Scheduled'] = $rrow['Appointments Scheduled'];
...
$JOpportunities = $agentScoreObj->JsonKeyValueConvertion($Opportunities, 'number');
$JAppointmentsScheduled = $agentScoreObj->JsonKeyValueConvertion($AppointmentsScheduled, 'number');
...
?>
		...
		var chart;$(document).ready(function(){chart=new Highcharts.Chart({chart:{renderTo:'container',marginRight:50,defaultSeriesType: 'pie'},credits:{enabled:false},title:{text:''},tooltip: {
                ...
                type: 'pie',
                name: 'CSAT',
                data: [<?php echo $JOpportunities;?>,<?php echo $JAppointmentsScheduled;?>]
            }]})});
