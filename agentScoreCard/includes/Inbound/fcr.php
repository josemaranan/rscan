...
<table width="97%" border="1" cellpadding="0" bgcolor="#FFFFFF" cellspacing="0" class="report table-autosort table-stripeclass:alternate"> 

<thead>
<tr>
    <th class="ColumnHeader1 locked">LOB Name</th>
    <th class="ColumnHeader1">FCR Goal</th>
    ...
    <?php } ?>
    <th class="ColumnHeader1">First Call <br />Resolution</th>
    <th class="ColumnHeader1">Repeat <br />Callers</th>
    <th class="ColumnHeader1">Total Calls</th>
		...
		if(!empty($requiredData))
		{
			...
			{ 
				...
				?>
							...
							if(!empty($requiredDataVal['FCR Goal']))
							{
								$agentScoreObj->numberFormat($requiredDataVal['FCR Goal']);
							}
							else
							{
								echo '&nbsp;';	
							...
							{?>
                                ...
                                if($requiredDataVal['LOB']=='Totals')
                                { ?>
                                         <img src="includes/images/pie_chart.png" border="0" style="width:20px; height:20px; cursor:hand;" title="Click here to view graph" onclick="return populateDetails('<?php echo $type;?>', '<?php echo $measure;?>' , '<?php echo $period;?>' , '<?php echo $employeID;?>', '<?php echo $startDate;?>' , '<?php echo $endDate;?>' , 'dataPie' , '' ); return false;" />
                                <?php }
							...
							{
								echo $requiredDataVal['First Call Resolution'];
							}
							else
							{
								echo '&nbsp;';	
								...
								?></td>
                            <td style="text-align:center;"><?php 
							if(!empty($requiredDataVal['Repeat Callers']))
							{
								echo $requiredDataVal['Repeat Callers'];
							}
							else
							{
								echo '&nbsp;'; 	
							...
							?></td>
							<td style="text-align:center;"><?php 
							if(!empty($requiredDataVal['Total Calls']))
							{
								echo $requiredDataVal['Total Calls'];
							}
							else
							{
								echo '&nbsp;';
								...
								?></td>
							<td style="text-align:center;"><?php 
							if(!empty($requiredDataVal['FCR']))
							{
								$agentScoreObj->numberFormat($requiredDataVal['FCR']);
							}
							else
							{
								echo '&nbsp;';	
